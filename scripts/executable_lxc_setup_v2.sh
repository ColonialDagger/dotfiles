...
# bash -c "$(curl -fsSL https://termbin.com/h0eo)"
...
extra_packages=(
    btop htop sudo fzf ncdu curl vim magic-wormhole cron net-tools
)
...
BASHRC_URL="https://termbin.com/31cy"
...
if $ANS_NVIDIA; then
    echo ""
    wget https://us.download.nvidia.com/XFree86/Linux-x86_64/580.76.05/NVIDIA-Linux-x86_64-580.76.05.run -O /tmp/NVIDIA-Linux-x86_64-580.76.05.run
    chmod +x /tmp/NVIDIA-Linux-x86_64-580.76.05.run
    /tmp/NVIDIA-Linux-x86_64-580.76.05.run --no-kernel-module -no-questions --silent
